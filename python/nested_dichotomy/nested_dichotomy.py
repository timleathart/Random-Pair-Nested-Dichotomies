...
def flatten_nested_tuple(x):
    if isinstance(x, collections.Iterable):
        return [a for i in x for a in flatten_nested_tuple(i)]
    else:
        return [x]
...
class NestedDichotomy(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, structure=None):
        labels = np.unique(y)
        ...
        if structure is None:
            ...
            pos_labels = flatten_nested_tuple(structure[0])
            neg_labels = flatten_nested_tuple(structure[1])
        ...
        self.left.fit(pos_X, pos_y, structure=structure[0])
        self.right.fit(neg_X, neg_y, structure=structure[1])

        # make sure structure is updated
        self.structure = (self.left.structure, self.right.structure)
    ...
    def _random_pair_selection(self, labels, X, y):
        ...
        for y_val in labels:
            ...
            if cm[y_val][0] < cm[y_val][1]:
                pos_labels.append(y_val)
            else:
                neg_labels.append(y_val)
